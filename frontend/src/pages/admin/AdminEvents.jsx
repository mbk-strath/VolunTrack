...
const AdminEvents = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const data = response.data || [];        if (data.length === 0) {
  ...
  if (loading) return <h3 className="no-events">Loading events...</h3>;
    ...
    <div className="EventsPage">
      ...
      {events.map((event) => (
          ...
          <div className="event-info">
            ...
            {event.start_date ? new Date(event.start_date).toLocaleDateString() : "—"} <br />
            <strong>End Date:</strong>{" "}
            {event.end_date ? new Date(event.end_date).toLocaleDateString() : "—"} <br />
            <strong>Description:</strong> {event.description || "No description"} <br />
            <strong>Required Skills:</strong> {event.required_skills || "N/A"} <br />
            <strong>Volunteers Needed:</strong> {event.num_volunteers_needed || "N/A"} <br />
